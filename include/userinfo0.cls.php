...
class cls_userinfo{
	...
	function currentuser(){
		...
		if($msid){
			...
			if($msession = $db->fetch_one($sqlstr)){
				...
				}else $msession = array_merge($msession,$nouserinfos);
			}
		}
		if(!$sessionexists){
			if($memberid){
			...
			if(empty($msession['mid'])) $msession = array_merge($msession,$nouserinfos);
		}
	...
	function updatesession(){
		...
		if($onlinetimecircle && $this->info['mid'] && $this->info['mslastactive'] && $timestamp - (!empty($this->info['lastolupdate']) ? $this->info['lastolupdate'] : $this->info['mslastactive']) > $onlinetimecircle * 60){
			$lastolupdate = $timestamp;
			$db->query("UPDATE {$tblprefix}onlinetime SET 
						totaltime=totaltime+'$onlinetimecircle', 
						monthtime=monthtime+'$onlinetimecircle', 
						lastupdate='$timestamp' 
						WHERE mid='{$this->info['mid']}' AND lastupdate<='".($timestamp - $onlinetimecircle * 60)."'");
			if(!$db->affected_rows()) {
				$db->query("INSERT INTO {$tblprefix}onlinetime (mid,monthtime,totaltime,lastupdate)
					VALUES ('{$this->info['mid']}', '$onlinetimecircle', '$onlinetimecircle', '$timestamp')", 'SILENT');
			}
		...
		if($sessionexists){
			...
			$db->query("DELETE FROM {$tblprefix}msession 
						WHERE msid='{$this->info['msid']}' 
						OR mslastactive<($timestamp-$onlinehold) 
						OR (mid<>'0' AND mid='{$this->info['mid']}') 
						OR (mid='0' AND onlineip='$onlineip' AND mslastactive>$timestamp-60)");
	
			$db->query("INSERT INTO {$tblprefix}msession (msid,onlineip,mid,mname,mslastactive,lastolupdate)
				VALUES ('{$this->info['msid']}','$onlineip','{$this->info['mid']}','{$this->info['mname']}','$timestamp','$lastolupdate')", 'SILENT');
		
			if($this->info['mid'] && $timestamp - $this->info['lastactive'] > 21600) {
				if($onlinetimecircle && $timestamp - $this->info['lastactive'] > 86400){
					$stronlinetime = ',onlinetime='.round(intval($db->result_one("SELECT totaltime FROM {$tblprefix}onlinetime WHERE mid='{$this->info['mid']}'")) / 60);
				}else $stronlinetime = '';
	...
	function pmbypmids($pname,$pmids=0){
		...
		return count(array_intersect($ugids,$nugids)) ? 1 : 0;
	}
