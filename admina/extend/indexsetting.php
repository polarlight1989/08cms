...
if($action == 'zh'){
	if(!submitcheck('bmconfigs')){
		tabheader('首页设置','cfsite',"?entry=extend&extend={$extend}&action=zh$param_suffix",2,1);
		if(!$sid){
			
			trbasic(lang('hosturl'),'mconfigsnew[hosturl]',$mconfigs['hosturl'],'btext',lang('aghosturl'));
			trbasic('站点名称','mconfigsnew[hostname]',$mconfigs['hostname'],'btext');
			trspecial('logo','mconfigsnew[logo]',$mconfigs['logo'],'image');
			trspecial('顶部背景图','mconfigsnew[index_topbg]',$mconfigs['index_topbg'],'image');
			trspecial('最新动态图片','mconfigsnew[index_newsimg]',$mconfigs['index_newsimg'],'image','','宽：572px，高：294px');
			trspecial('精彩回顾图片','mconfigsnew[index_reviewimg]',$mconfigs['index_reviewimg'],'images','0','宽：221px，高：166px');
			
			?>
			<script src="include/js/jquery.js"></script>
			<script>
			var year = '<?=date("Y")?>';
	var month = '<?=(int)date('m')?>';
 
 
	$(function(){
		$("[rqday]").click(function(){
			if(parseInt($(this).html())){
				var val = "'"+year+"-"+month+"-"+$(this).html()+"'";
				
				if($(this).hasClass('rqred')){
					$(this).removeClass('rqred');
					$("#rq_val").val($("#rq_val").val().replace(val,''));
				}else{
					$(this).addClass('rqred');
					$("#rq_val").val($("#rq_val").val()+val);
				}
			...
			<script src="/public/js/rq.js"></script>
			<style>
			.rqtable td{
			...
			<tr><td width="25%" class="txt txtright fB borderright">课程时刻表</td>
<td class="txt txtleft">
<div style="font-size:14px;" style='width:300px;'>
<table width="300px" style='margin-left:20px;' border="0" class='rqtable'  cellpadding="0" cellspacing="1" bgcolor="d0d0d0">
  <tr>
    <td height="30" align="center" valign="middle" bgcolor="#FFFFFF"><strong>日</strong></td>
    <td height="30" align="center" valign="middle" bgcolor="#FFFFFF"><strong>一</strong></td>
    <td height="30" align="center" valign="middle" bgcolor="#FFFFFF"><strong>二</strong></td>
    <td height="30" align="center" valign="middle" bgcolor="#FFFFFF"><strong>三</strong></td>
    <td height="30" align="center" valign="middle" bgcolor="#FFFFFF"><strong>四</strong></td>
    <td height="30" align="center" valign="middle" bgcolor="#FFFFFF"><strong>五</strong></td>
    <td height="30" align="center" valign="middle" bgcolor="#FFFFFF"><strong>六</strong></td>
  </tr>
  <?php
  for($i=0;$i<6;$i++){
  ?>
  <tr>
    <td height="30" align="center" valign="middle" rqday bgcolor="#FFFFFF" >&nbsp;</td>
    <td height="30" align="center" valign="middle" rqday bgcolor="#FFFFFF" >&nbsp;</td>
    <td height="30" align="center" valign="middle" rqday bgcolor="#FFFFFF" >&nbsp;</td>
    <td height="30" align="center" valign="middle" rqday bgcolor="#FFFFFF" >&nbsp;</td>
    <td height="30" align="center" valign="middle" rqday bgcolor="#FFFFFF" >&nbsp;</td>
  ...
  <?php } ?>
	<input type="hidden" size="50" id="rq_val" name="mconfigsnew[rq]" value="<?=$mconfigs['rq']?>">
</table>
</div>
<div style="padding:10px 0px;width:300px;margin-left:20px;"><table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tbody><tr>
    <td height="30" align="center" valign="middle"><a rqprev="" href="javascript:void(0);"><img src="/public/images/b1.png" width="13" height="14"></a></td>
    <td height="30" align="center" valign="middle"><strong rqshow=""></strong></td>
    <td height="30" align="center" valign="middle"><a rqnext="" href="javascript:void(0);"><img src="/public/images/b2.png" width="13" height="14"></a></td>
  </tr>
</tbody></table>
</div>
</td></tr>
			<?php
			trbasic('关于我们 - 文字','mconfigsnew[aboutus_text]',$mconfigs['aboutus_text'],'textarea');
			trspecial('关于我们 - 图片','mconfigsnew[aboutus_img]',$mconfigs['aboutus_img'],'image','','宽：256px，高：86px');
			 
			trspecial('二维码图片','mconfigsnew[wximg]',$mconfigs['wximg'],'image','','宽：120px，高：120px');
			trbasic('版权信息','mconfigsnew[copyright]',$mconfigs['copyright'],'textarea');
			trbasic('企业邮箱','mconfigsnew[qyyx]',$mconfigs['qyyx'],'btext');
			trspecial('我们的客户','mconfigsnew[kehu]',$mconfigs['kehu'],'images','0','宽：90px，高：33px');
			trbasic('在线留言邮箱','mconfigsnew[liuyanEmail]',$mconfigs['liuyanEmail'],'btext','支持多个，请用"|"分隔');
			tabfooter('');
		...
		if(!$sid){
			...
			if($mconfigsnew['index_reviewimg']){
				$mconfigsnew['index_reviewimg'] = str_replace("|","",$mconfigsnew['index_reviewimg']);
				$v = explode("\r\n",$mconfigsnew['index_reviewimg']);
				
				$result = array();
				if($v){
					foreach($v as $vv){
						$result[]['remote'] = trim($vv);
					}
				}
				$mconfigsnew['index_reviewimg'] = serialize($result);
 
			}
			if($mconfigsnew['kehu']){
				$mconfigsnew['kehu'] = str_replace("|","",$mconfigsnew['kehu']);
				$v = explode("\r\n",$mconfigsnew['kehu']);
				...
				$mconfigsnew['kehu'] = serialize($result);
...
if($action == 'mzh'){
	if(!submitcheck('bmconfigs')){
		tabheader('首页设置','cfsite',"?entry=extend&extend={$extend}&action=mzh$param_suffix",2,1);
		if(!$sid){
			trspecial('logo','mconfigsnew[mzh_logo]',$mconfigs['mzh_logo'],'image');
			trbasic('公司简介标签','mconfigsnew[zh_maboutus]',$mconfigs['zh_maboutus'],'textarea');
			 
			trbasic('版权信息','mconfigsnew[zh_mcopyright]',$mconfigs['zh_mcopyright'],'textarea');
			tabfooter('');
 
			tabheader('幻灯片');
			trspecial('幻灯片1','mconfigsnew[zh_mfocus1_img]',$mconfigs['zh_mfocus1_img'],'image');
			trbasic('幻灯片1链接','mconfigsnew[zh_mfocus1_href]',$mconfigs['zh_mfocus1_href'],'btext');
			trspecial('幻灯片2','mconfigsnew[zh_mfocus2_img]',$mconfigs['zh_mfocus2_img'],'image');
			trbasic('幻灯片2链接','mconfigsnew[zh_mfocus2_href]',$mconfigs['zh_mfocus2_href'],'btext');
			trspecial('幻灯片3','mconfigsnew[zh_mfocus3_img]',$mconfigs['zh_mfocus3_img'],'image');
			trbasic('幻灯片3链接','mconfigsnew[zh_mfocus3_href]',$mconfigs['zh_mfocus3_href'],'btext');
			trspecial('幻灯片4','mconfigsnew[zh_mfocus4_img]',$mconfigs['zh_mfocus4_img'],'image');
			trbasic('幻灯片4链接','mconfigsnew[zh_mfocus4_href]',$mconfigs['zh_mfocus4_href'],'btext');
			trspecial('幻灯片5','mconfigsnew[zh_mfocus5_img]',$mconfigs['zh_mfocus5_img'],'image');
			trbasic('幻灯片5链接','mconfigsnew[zh_mfocus5_href]',$mconfigs['zh_mfocus5_href'],'btext');
			tabfooter('');
			
			tabheader('全部课程 - 图片 【 无图则不在首页显示 】');
			$q = $db->query("SELECT aid,subject FROM {$tblprefix}archives WHERE caid = '4'");
			while($v = $db->fetch_array($q)){
				$vname = "aid_".$v['aid']."_mthumb";
				trspecial($v['subject'],'mconfigsnew['.$vname.']',$mconfigs[$vname],'image');
			}
			
			 
			tabfooter('');
			tabheader('假日课程 - 图片 【 无图则不在首页显示 】');
			$q = $db->query("SELECT caid,title FROM {$tblprefix}catalogs WHERE pid = '5'");
			while($v = $db->fetch_array($q)){
				$vname = "caid_".$v['caid']."_mthumb";
				trspecial($v['title'],'mconfigsnew['.$vname.']',$mconfigs[$vname],'image');
			}
...
if($action == 'men'){
	if(!submitcheck('bmconfigs')){
		tabheader('首页设置','cfsite',"?entry=extend&extend={$extend}&action=men$param_suffix",2,1);
		if(!$sid){
			trspecial('logo','mconfigsnew[men_logo]',$mconfigs['men_logo'],'image');
			trbasic('公司简介标签','mconfigsnew[en_maboutus]',$mconfigs['en_maboutus'],'textarea');
			 
			trbasic('版权信息','mconfigsnew[en_mcopyright]',$mconfigs['en_mcopyright'],'textarea');
			tabfooter('');
 
			tabheader('幻灯片');
			trspecial('幻灯片1','mconfigsnew[en_mfocus1_img]',$mconfigs['en_mfocus1_img'],'image');
			trbasic('幻灯片1链接','mconfigsnew[en_mfocus1_href]',$mconfigs['en_mfocus1_href'],'btext');
			trspecial('幻灯片2','mconfigsnew[en_mfocus2_img]',$mconfigs['en_mfocus2_img'],'image');
			trbasic('幻灯片2链接','mconfigsnew[en_mfocus2_href]',$mconfigs['en_mfocus2_href'],'btext');
			trspecial('幻灯片3','mconfigsnew[en_mfocus3_img]',$mconfigs['en_mfocus3_img'],'image');
			trbasic('幻灯片3链接','mconfigsnew[en_mfocus3_href]',$mconfigs['en_mfocus3_href'],'btext');
			trspecial('幻灯片4','mconfigsnew[en_mfocus4_img]',$mconfigs['en_mfocus4_img'],'image');
			trbasic('幻灯片4链接','mconfigsnew[en_mfocus4_href]',$mconfigs['en_mfocus4_href'],'btext');
			trspecial('幻灯片5','mconfigsnew[en_mfocus5_img]',$mconfigs['en_mfocus5_img'],'image');
			trbasic('幻灯片5链接','mconfigsnew[en_mfocus5_href]',$mconfigs['en_mfocus5_href'],'btext');
			tabfooter('');
			
			tabheader('全部课程 - 图片 【 无图则不在首页显示 】');
			$q = $db->query("SELECT aid,subject FROM {$tblprefix}archives WHERE caid = '23'");
			while($v = $db->fetch_array($q)){
				$vname = "aid_".$v['aid']."_mthumb";
				trspecial($v['subject'],'mconfigsnew['.$vname.']',$mconfigs[$vname],'image');
			}
			...
			tabfooter('');
			tabheader('假日课程 - 图片 【 无图则不在首页显示 】');
			$q = $db->query("SELECT caid,title FROM {$tblprefix}catalogs WHERE pid = '24'");
			while($v = $db->fetch_array($q)){
				$vname = "caid_".$v['caid']."_mthumb";
				trspecial($v['title'],'mconfigsnew['.$vname.']',$mconfigs[$vname],'image');
			}
			trspecial('背景图','mconfigsnew[en_jrkc_bg]',$mconfigs['en_jrkc_bg'],'image');
			?>
			<tr><td width="25%" class="txt txtright fB borderright">背景图PSD</td>
			<td class="txt txtleft"><a href="/public/images/en_HolidayCourses.psd">下载</a></td></tr>
			<?php
...
if($action == 'en'){
	if(!submitcheck('bmconfigs')){
		tabheader('首页设置','cfsite',"?entry=extend&extend={$extend}&action=en$param_suffix",2,1);
		if(!$sid){
			
			 
			trbasic('站点英文名称','mconfigsnew[encmsname]',$mconfigs['encmsname']);
			trspecial('logo','mconfigsnew[en_logo]',$mconfigs['en_logo'],'image');
			trbasic('关于我们标签','mconfigsnew[en_aboutus]',$mconfigs['en_aboutus'],'textarea');
			trbasic('视频地址','mconfigsnew[en_shipin]',$mconfigs['en_shipin'],'textarea');
			trbasic('版权信息','mconfigsnew[en_copyright]',$mconfigs['en_copyright'],'textarea');
			tabfooter('');
			tabheader('PC端幻灯片');
			trspecial('幻灯片1','mconfigsnew[en_focus1_img]',$mconfigs['en_focus1_img'],'image');
			trbasic('幻灯片1链接','mconfigsnew[en_focus1_href]',$mconfigs['en_focus1_href'],'btext');
			trspecial('幻灯片2','mconfigsnew[en_focus2_img]',$mconfigs['en_focus2_img'],'image');
			trbasic('幻灯片2链接','mconfigsnew[en_focus2_href]',$mconfigs['en_focus2_href'],'btext');
			trspecial('幻灯片3','mconfigsnew[en_focus3_img]',$mconfigs['en_focus3_img'],'image');
			trbasic('幻灯片3链接','mconfigsnew[en_focus3_href]',$mconfigs['en_focus3_href'],'btext');
			trspecial('幻灯片4','mconfigsnew[en_focus4_img]',$mconfigs['en_focus4_img'],'image');
			trbasic('幻灯片4链接','mconfigsnew[en_focus4_href]',$mconfigs['en_focus4_href'],'btext');
			trspecial('幻灯片5','mconfigsnew[en_focus5_img]',$mconfigs['en_focus5_img'],'image');
			trbasic('幻灯片5链接','mconfigsnew[en_focus5_href]',$mconfigs['en_focus5_href'],'btext');
			tabheader('假日课程图片');
