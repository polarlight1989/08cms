...
switch($url_type){
	case 'tpl':
		...
		!$sid && $urlsarr['mcnodes'] = array('>'.lang('mcnode'),"?entry=mcnodes&action=mcnodesedit$param_suffix");
	break;
	case 'othertpl':
		$urlsarr = array(
	...
	case 'faces':
		$urlsarr = array(
		'face' => array(lang('faceadmin'),"?entry=faces$param_suffix"),
		'update' => array(lang('updateface'),"?entry=faces&action=update$param_suffix"),
		);
	break;
	case 'repus':
		$urlsarr = array(
	...
	case 'channeldetail':
		...
		'album' => array(lang('abandcu'),"?entry=channels&action=channelalbum&chid=$chid$param_suffix"),
		'other' => array(lang('otherset'),"?entry=channels&action=channelother&chid=$chid$param_suffix"),
		'adv' => array(lang('advsetting'),"?entry=channels&action=channeladv&chid=$chid$param_suffix"),
		);
	break;
	case 'cnode';
		$urlsarr = array(
		'cnodescommon' => array(lang('mcnodemanager'),"?entry=cnodes&action=cnodescommon$param_suffix"),
	...
	case 'mcnode';
		$urlsarr = array(
		'mcnodesedit' => array(lang('mcnodeadmin'),"?entry=mcnodes&action=mcnodesedit$param_suffix"),
		'mcnodeadd' => array(lang('addmcnode'),"?entry=mcnodes&action=mcnodeadd$param_suffix"),
		'tpl' => array('>'.lang('othertpl'),"?entry=tplconfig&action=tplbase$param_suffix"),
		);
	break;
	case 'cufield':
	...
	case 'mconfig':
		...
		if(!$sid) $urlsarr['cfmobmail'] = array(lang('mob_mail'),"?entry=mconfigs&action=cfmobmail$param_suffix");
	...
	case 'static':
		...
		if(!$sid) $urlsarr['mcnodes'] = array(lang('mcn_static'),"?entry=static&action=mcnodes$param_suffix");
		$urlsarr['freeinfos'] = array(lang('isolute_page_manager'),"?entry=freeinfos&action=freeinfosedit$param_suffix");
	break;
	case 'subsite':
		$urlsarr = array(
	...
	case 'wap':
		$urlsarr = array(
		'set' => array(lang('wap_set'),"?entry=wap"),
		'lang' => array(lang('wap_lang'),"?entry=wap&action=lang"),
		);
	break;
	case 'memcert':
		$urlsarr = array(
		'' => array(lang('memcert_list'),"?entry=$entry$param_suffix"),
		'memcerts' => array(lang('memcert_admin'),"?entry=$entry&action=memcerts$param_suffix"),
		'add' => array(lang('memcert_add'),"?entry=$entry&action=add$param_suffix"),
		'cfmobmail' => array(lang('mob_mail'),'?entry=mconfigs&action=cfmobmail'),
		'email' => array(lang('emaltpl'),"?entry=splangs&action=splangsedit"),
		);
	break;
}
